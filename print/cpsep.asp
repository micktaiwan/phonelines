...
<%
   ...
   obj.NewQuery("SELECT visits.*, personnel.CODE AS TCODE FROM visits LEFT OUTER JOIN dispatch ON visits.REASON=dispatch.NAME LEFT OUTER JOIN personnel ON visits.TEAM=personnel.TEAM WHERE visits.RESULT='隔日'  AND "+str+cond1+" ORDER BY visits.TEAM");
   obj.NewTemplate(SitePath+"print\\sepprint.wet");
