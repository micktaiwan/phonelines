...
Response.Write("<b>退件清單</b><br>");
var str="";
if(zone!="") {
   str = "visits.ZONE = '"+zone+"'"
...
if(datefrom!="") {
   if(str!="") str += " AND ";
   str += "visits.DATE >= '"+datefrom+"'"
   Response.Write("從日期: "+datefrom+"<br>");
   }
if(dateto!="") {
   if(str!="") str += " AND ";
   str += "visits.DATE <= '"+dateto+"'"
...
<%
   ...
   var sql = "SELECT DISTINCT visits.team, visits.phone, visits.serial, visits.reason, visits.memo, cancel.CODE, personnel.CODE AS TCODE FROM visits LEFT OUTER JOIN cancel ON visits.REASON=cancel.NAME LEFT OUTER JOIN personnel ON visits.TEAM=personnel.TEAM WHERE visits.RESULT='退件'  AND "+str+cond1+" ORDER BY cancel.CODE";
   obj.NewQuery(sql);
   
   obj.NewTemplate(SitePath+"print\\cancelprint.wet");
