...
<%
      ...
      obj.NewQuery("SELECT "+isnew+", repairlog.*, ISNULL(repairlog.VISITID,'') AS VISITID, visits.TEAM, visits.DATE AS VISDATE, visits.ZONE FROM repairlog LEFT OUTER JOIN visits ON repairlog.VISITID=visits.ID WHERE ISNULL(CORRECTED,0)=0 "+cond1+" ORDER BY REPORTDATE DESC");
